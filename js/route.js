...
function evaluateStepWind(step, latlngs) {
    ...
    // Correction locale de 180° pour l'annonce vocale
    const rawWindDir = window.currentWindDirection || 0;
    const windDir = (rawWindDir + 180) % 360;
...
async function getRoute() {
    ...
    if (normalFeature.properties && normalFeature.properties.segments) {
        ...
        window.normSteps = prepareStepsForVoice(rawSteps, latlngsNormal);
    ...
    if (alternativeFeature.properties && alternativeFeature.properties.segments) {
        ...
        window.altSteps = prepareStepsForVoice(rawAltSteps, latlngsAlternative);
    }
...
async function recalculateRouteFromDeviation(lat, lon) {
    ...
    try {
        ...
        if (normalFeature.properties && normalFeature.properties.segments) {
            window.normSteps = prepareStepsForVoice(normalFeature.properties.segments[0].steps, latlngsNormal);
        }
        if (alternativeFeature.properties && alternativeFeature.properties.segments) {
            window.altSteps = prepareStepsForVoice(alternativeFeature.properties.segments[0].steps, latlngsAlternative);
        }
